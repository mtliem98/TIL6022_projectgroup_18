...
GTFS_PATH = os.path.join(SCRIPT_DIR, "gtfs-nl\\")

stops = pd.read_csv(GTFS_PATH + "stops.txt", low_memory=False)
stop_times = pd.read_csv(GTFS_PATH + "stop_times.txt", low_memory=False)
trips = pd.read_csv(GTFS_PATH + "trips.txt", low_memory=False)
routes = pd.read_csv(GTFS_PATH + "routes.txt", low_memory=False)
...
# exporting to csv
df_grouped.to_csv(os.path.join(SCRIPT_DIR, "median_travel_times.csv"), index=False)
